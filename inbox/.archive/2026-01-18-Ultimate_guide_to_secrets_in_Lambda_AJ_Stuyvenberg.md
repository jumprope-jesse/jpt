...
type: link
source: notion
url: https://aaronstuyvenberg.com/posts/ultimate-lambda-secrets-guide
notion_type: Tech Deep Dive
tags: ['Running']
created: 2024-04-06T09:04:00.000Z
---

# Ultimate guide to secrets in Lambda | AJ Stuyvenberg

## AI Summary (from Notion)
- Purpose: The document serves as a comprehensive guide on managing secrets in AWS Lambda, focusing on securing sensitive information like API keys and database passwords.

- Key Options for Secret Management:
- Lambda Environment Variables:
- Easiest to use but not the most secure option.
- Encrypted at rest but visible in plain text when accessed.
- AWS Systems Manager Parameter Store:
- Offers Standard and Advanced parameters; Advanced allows for expiration policies.
- AWS Secrets Manager:
- Specifically designed for managing secrets, allows automatic rotation and replication across regions.
- AWS Key Management Service (KMS):
- Underpins other services, used for encrypting secrets, but requires more manual management.

- Best Practices:
- Avoid hardcoding plaintext secrets in application code.
- Use the principle of least privilege; limit access to secrets based on need.
- Implement secure storage solutions and use dynamic references in CloudFormation to avoid exposing secrets.

- Performance Insights:
- Environment variables add minimal latency but can expose secrets during development.
- Parameter Store and Secrets Manager have higher latency due to connection setup.

- Major Themes:
- Security vs. Usability: Balancing ease of access to secrets with the need to secure sensitive information.
- Cost Management: Evaluating the costs associated with different secret management strategies, especially in high-volume environments.

- Hot Takes:
- Lambda environment variables are generally acceptable for low-risk secrets but less suitable for sensitive data.
- Secrets Manager, while more expensive, provides peace of mind for applications needing frequent credential rotation.

- Interesting Facts:
- Secrets Manager's costs can accumulate quickly depending on the number of regions and secrets.
- Using envelope encryption can enhance security without significantly increasing costs.

- Concluding Thoughts:
- The guide emphasizes the need for a tailored approach to secret management based on the specific use case, with a strong recommendation to avoid simplistic solutions for more sensitive data.
...
1. AWS Systems Manager Parameter Store (Formerly known as Simple Systems Manager, or SSM)
...
Systems Manager Parameter Store cold request

With an existing connection, fetching the parameter took around 39.3ms:

Systems Manager Parameter Store warm request
...
1. Store your secrets as SecureStrings in Systems Manager Parameter Store.
1. Use CloudFormation dynamic references to pass a reference to your secret to CloudFormation.
...
1. Create a KMS key
1. Generate 256-bit AES key for each customer, application, or secrets payload
1. Encrypt all of your secrets with the AES key. This is the “envelope”
1. Include the encrypted secrets in your function zip.
1. Finally, encrypt the AES key with your KMS key and pass the encrypted key to your function in an environment variable.
