...
tags: ['Running']
created: 2025-11-02T13:11:00.000Z
---

# How I Use Every Claude Code Feature - by Shrivu Shankar

## Overview (from Notion)
- Embrace the power of tools like Claude Code to streamline your software development process, especially as you juggle family responsibilities and entrepreneurship.
- The emphasis on maintaining a clean CLAUDE.md can parallel your life management—keeping things organized helps in both code and personal life.
- Consider the balance between automation and human oversight; as a founder, you might find it useful to delegate tasks to AI while ensuring quality through human checks.
- The idea of using hooks for validation can inspire methods to implement checks in family routines or business processes, ensuring everything runs smoothly.
- Explore the notion of "shoot and forget" with AI; it could resonate with how you prioritize tasks at home and work.
- The use of planning modes can help in setting goals for both your professional projects and family activities, aligning efforts efficiently.
- The discussion around skills and scripting may prompt you to think about teaching your children programming or encouraging their creativity through technology.
- Consider the philosophical implications of AI-generated work, especially how it affects collaboration; it may shape how you think about teamwork in your ventures.
- The insights on customizing settings for efficiency can apply to personalizing family routines or optimizing your work environment.

## AI Summary (from Notion)
The author shares insights on using Claude Code, emphasizing the importance of the CLAUDE.md file as a foundational document for projects. Key practices include maintaining concise documentation, utilizing slash commands as shortcuts, and leveraging subagents for context management. The author advocates for a planning mode for complex changes and highlights the utility of the Claude Code SDK for scripting and building internal tools. Additionally, the integration of Claude Code with GitHub Actions is praised for enhancing operational efficiency and auditability. Overall, the piece serves as a comprehensive guide for maximizing the effectiveness of Claude Code in various workflows.
...
1. Don’t Just Say “Never.” Avoid negative-only constraints like “Never use the -foo-bar flag.” The agent will get stuck when it thinks it must use that flag. Always provide an alternative.
1. Use CLAUDE.md as a Forcing Function. If your CLI commands are complex and verbose, don’t write paragraphs of documentation to explain them. That’s patching a human problem. Instead, write a simple bash wrapper with a clear, intuitive API and document that. Keeping your CLAUDE.md as short as possible is a fantastic forcing function for simplifying your codebase and internal tooling.
...
1. They Force Human Workflows: Worse, they force Claude into a rigid, human-defined workflow. I’m now dictating how it must delegate, which is the very problem I’m trying to get the agent to solve for me.
...
1. Hint Hooks: These are simple, non-blocking hooks that provide “fire-and-forget” feedback if the agent is doing something suboptimal.
...
1. Tool Calling: The “classic” agent model. We hand-craft tools and abstract away reality for the agent. (Better, but creates new abstractions and context bottlenecks).
1. Scripting: We give the agent access to the raw environment—binaries, scripts, and docs—and it writes code on the fly to interact with them.
...
1. Building Internal Chat Tools: The SDK is perfect for wrapping complex processes in a simple chat interface for non-technical users. Like an installer that, on error, falls back to the Claude Code SDK to just fix the problem for the user. Or an in-house “v0-at-home” tool that lets our design team vibe-code mock frontends in our in-house UI framework, ensuring their ideas are high-fidelity and the code is more directly usable in frontend production code.
1. Rapid Agent Prototyping: This is my most common use. It’s not just for coding. If I have an idea for any agentic task (e.g., a “threat investigation agent” that uses custom CLIs or MCPs), I use the Claude Code SDK to quickly build and test the prototype before committing to a full, deployed scaffolding.
