...
type: link
source: notion
url: https://github.com/Adawg4/openapi-autospec
notion_type: Software Repo
tags: ['Running']
created: 2024-03-26T02:42:00.000Z
---

# GitHub - Adawg4/openapi-autospec: Proxy server that generates API specs for any app or website on localhost.

## AI Summary (from Notion)
- Project Overview: OpenAPI AutoSpec is a proxy server that generates OpenAPI specifications in real-time from locally running websites or applications.
- Key Features:
- Automatically generates OpenAPI 3.0 specifications.
- Captures and documents requests and responses, including headers and bodies.
- Provides real-time review of specifications in the terminal and allows easy export.
- Ignores static file URLs for cleaner documentation.
- Installation:
- Requires Node.js (0.10 or higher) and can be installed via npm with the command $ npm install openapi-autospec.
- Usage Instructions:
- Start the server using the command $ npx autospec --portTo PORT --portFrom PORT --filePath openapi.json.
- Automatically documents API behavior based on observed network requests.
- Allows customization of exported documentation by replacing localhost information.
- Roadmap: Future enhancements include support for HTTPS, OpenAPI 3.1, and tools for parameterization and running on various cloud services.
- OpenAPI Significance: Provides a standardized way to document APIs, making them easier to understand and integrate.
- Community Engagement: Encourages users to join their Discord for support and contributions to the project.
- Multiple Server Proxies: Instructions for running proxies for full-stack frameworks and separate frontend/backend applications.
- Backend Port Changes: Guides on how to change server ports for popular frameworks like Express, Django, and Rails.
- Interesting Fact: The tool is targeted at developers looking to streamline API documentation without complex setup processes.
...
1. Run the script and specify the localhost information where traffic should be routed. Ex. npx autospec --portTo 8000.
```plain text
$ npx autospec --portTo PORT --portFrom PORT--filePath openapi.json
...
1. Access the newly created file - named with the current timestamp.
