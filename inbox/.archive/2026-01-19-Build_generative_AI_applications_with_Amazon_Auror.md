...
tags: ['Running']
created: 2024-02-13T16:28:00.000Z
---

# Build generative AI applications with Amazon Aurora and Knowledge Bases for Amazon Bedrock | AWS Database Blog

## AI Summary (from Notion)
- Overview of Amazon Bedrock: Amazon Bedrock is a platform for building and scaling generative AI applications using foundational models (FMs) trained on extensive datasets.
- Retrieval Augmented Generation (RAG): RAG is a technique that combines FMs with private data stored in services like Amazon S3 and Amazon Aurora to provide relevant answers.
- Knowledge Bases for Amazon Bedrock: A fully managed RAG capability that customizes FM responses with company data, automating the end-to-end RAG workflow.
- Integration with Amazon Aurora: Aurora PostgreSQL enhances vector similarity search performance for RAG, delivering queries significantly faster.
- Setting Up: Instructions are provided for setting up an Aurora cluster as a knowledge base, including the creation of an S3 bucket for data storage, configuring the Aurora cluster, and creating a secret in AWS Secrets Manager.
- Generating Embeddings: The article explains how to use Aurora ML to generate embeddings for data stored in Aurora, allowing for efficient querying and integration with Amazon Bedrock.
- IAM Role and Policy: Steps are outlined for creating an IAM role and policy that allows Aurora to communicate with Amazon Bedrock.
- Cleaning Up Resources: Guidance is given for deleting resources once they are no longer needed, including Aurora clusters and knowledge bases.
- Authors: The article is authored by Steve Dille and Jonathan Katz, both of whom have significant experience in product management and technical roles related to AWS and PostgreSQL.
- Key Takeaway: RAG is presented as a powerful approach to enhance AI applications by integrating domain-specific information with foundational models, enriching the responses and improving the relevance of generated content.
