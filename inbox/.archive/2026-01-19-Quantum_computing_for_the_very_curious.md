...
tags: ['Running']
created: 2024-02-18T07:01:00.000Z
---

# Quantum computing for the very curious

## AI Summary (from Notion)
- Overview of Quantum Computing: The document introduces quantum computing as a field focused on using quantum mechanics principles for computation, emphasizing its potential to outperform classical computing.

- Key Concepts:
- Qubits: The fundamental unit of quantum information, analogous to bits in classical computing.
- Quantum Gates: Operators that manipulate qubits, including NOT gates and Hadamard gates, enabling complex computations.
- Superposition and Entanglement: Concepts that allow qubits to exist in multiple states simultaneously and to be interdependent, respectively.

- Quantum Algorithms:
- Shor's Algorithm: A quantum algorithm for factoring large integers efficiently, posing risks to classical encryption methods.
- Quantum Search Algorithms: Techniques that may provide speed advantages over classical search methods.

- Comparison to Classical Computing:
- Quantum computers can simulate quantum systems efficiently, while classical computers struggle with such tasks.
- The document discusses the limitations of classical circuits when dealing with quantum phenomena.

- Potential Applications:
- Drug Discovery and Material Science: Quantum simulation could revolutionize how pharmaceutical companies and researchers explore new molecules and materials.
- Cryptography: Understanding quantum computing's impact on encryption and data security.

- Theoretical Insights:
- The relationship between quantum mechanics and computation is explored, questioning the nature of information and its implications for understanding the universe.
- The document highlights the ongoing search for a unifying theory of quantum gravity and its implications on computational capacity.

- Future Perspectives:
- The text suggests that quantum technology might lead to transformative industries and societal changes over the next century.
- It emphasizes the importance of continued research and development in quantum computing to realize its full potential.

- Call to Action: Encourages readers to engage with the material actively, suggesting that understanding quantum principles is essential for future technological advancements.

## Content (from Notion)

Presented in a new mnemonic medium which makes it almost effortless to remember what you read.

- Quantum computing for the very curiousLong-term
...
Adam Wiggins, Andrew Sutherland, Bert Muthalaly, Calvin French-Owen, Dwight Crow, fnnch, James Hill-Khurana, Lambda AI Hardware, Ludwig Petersson, Mickey McManus, Mintter, Patrick Collison, Paul Sutter, Peter Hartree, Sana Labs, Shripriya Mahesh, Tim O'Reilly.
...
For the most part you don’t notice these differences when you use your computer. Computer designers work very, very hard to make the details of the physical instantiation of the bits invisible not just to the user, but also (often) invisible even to programmers. Many programmers never think about whether a bit is stored in fast on-microprocessor cache memory, in the dynamic RAM chips, or in some type of virtual memory (say, on a hard disk). There are exceptions – programmers working on high-performance programs sometimes do think about these things, to make their programs as fast as possible. But for many programmers it doesn’t much matter how bits are stored. Rather, they can think of the bit in purely abstract terms, as having a state which is either 00 or 11.

In a manner similar to the way conventional computers are made up of bits, quantum computers are made up of quantum bits, or qubits. Just like a bit, a qubit has a state. But whereas the state of a bit is a number (00 or 11), the state of a qubit is a vector. More specifically, the state of a qubit is a vector in a two-dimensional vector space. This vector space is known as state space. For instance, here’s a possible state for a qubit:
...
Let’s get back to understanding qubits. I’ve described what the state of a qubit is, but given no hint about how (or whether) that’s connected to the state of a classical bit. (Henceforth we’ll use the phrase “classical bit” instead of “conventional bit”, after “classical physics”). In fact, there are two special quantum states which correspond to the 00 and 11 states of a classical bit. The quantum state corresponding to 00 is usually denoted ∣0⟩∣0⟩. That’s a fancy notation for the following vector:

∣0⟩:=[10].∣0⟩:=[10].

This special state ∣0⟩∣0⟩ is called a computational basis state.

It’s tempting to see the ∣0⟩∣0⟩ notation and wonder what all the separate pieces mean – what, for instance, does the ∣∣ mean; what does the ⟩⟩ mean; and so on?

In fact, it’s best to regard ∣0⟩∣0⟩ as a single symbol, standing for a single mathematical object – that vector we just saw above, [10][10]. The ∣∣ and ⟩⟩ don’t really have separate meanings except to signify this is a quantum state. In this, ∣0⟩∣0⟩ is much like the symbol 00: both stand for a single mathematical entity. And, as we’ll gradually come to see, a quantum computer can manipulate ∣0⟩∣0⟩ in ways very similar to how a conventional computer can manipulate 00.

This notation with ∣∣ and ⟩⟩ is called the ket notation, and things like ∣0⟩∣0⟩ are called kets. But don’t be thrown off by the unfamiliar terminology – a ket is just a vector, and when we say something is a ket, all we mean is that it’s a vector.

That said, the term ket is most often used in connection with notations like ∣0⟩∣0⟩. That is, we wouldn’t usually refer to [10][10] as a ket; we’d call it a vector, instead. If you’re a fan of really sharp definitions and strict consistency, this may seem vague and wishy-washy, but in practice doesn’t cause confusion. It’s just a convention to be aware of!

Alright, so ∣0⟩∣0⟩ is a computational basis state for a qubit, and plays much the same role as 00 does for a classical bit. It won’t surprise you to learn that there is another computational basis state, denoted ∣1⟩∣1⟩, which plays the same role as 11 does for a bit. Like ∣0⟩∣0⟩, ∣1⟩∣1⟩ is just a notation for a two-dimensional vector, in this case:

∣1⟩:=[01].∣1⟩:=[01].

Again, we’ll gradually come to see that ∣1⟩∣1⟩ has all the properties we expect of the 11 state of a classical bit.
...
The computational basis states ∣0⟩∣0⟩ and ∣1⟩∣1⟩ are just two possible states for a qubit. Many more states are possible, and those extra states endow qubits with capabilities not available to ordinary classical bits. In general, remember, a quantum state is a two-dimensional vector. Here’s an example, with a graphical illustration emphasizing the vector nature of the state:

In this example, the state 0.6∣0⟩+0.8∣1⟩0.6∣0⟩+0.8∣1⟩ is just 0.60.6 times the ∣0⟩∣0⟩ vector, plus 0.80.8 times the ∣1⟩∣1⟩ vector. In the usual vector notation that means the state is:

0.6∣0⟩+0.8∣1⟩=0.6[10]+0.8[01]=[0.60.8].0.6∣0⟩+0.8∣1⟩=0.6[10]+0.8[01]=[0.60.8].
...
One of the most common terms is superposition. People will say a state like 0.6∣0⟩+0.8∣1⟩0.6∣0⟩+0.8∣1⟩ is a superposition of ∣0⟩∣0⟩ and ∣1⟩∣1⟩. All they mean is that the state is a linear combination of ∣0⟩∣0⟩ and ∣1⟩∣1⟩. You may wonder why they don’t just say “linear combination” (and sometimes they do), but the reason is pretty much the same reason English-speakers say “hello” while Spanish-speakers say “hola” – the two terms come out of different cultures and different histories.

Another common term is amplitude. An amplitude is the coefficient for a particular state in superposition. For instance, in the state 0.6∣0⟩+0.8∣1⟩0.6∣0⟩+0.8∣1⟩ the amplitude for ∣0⟩∣0⟩ is 0.60.6, and the amplitude for ∣1⟩∣1⟩ is 0.80.8.

We’ve learnt that a quantum state is a two-dimensional complex vector. Actually, it can’t be just any old vector, a fact you might have guessed from the very particular amplitudes in some of the examples above. There’s a constraint. The constraint is this: the sums of the squares of the amplitudes must be equal to 11.

So, for example, for the state 0.6∣0⟩+0.8∣1⟩0.6∣0⟩+0.8∣1⟩ the sum of the squares of the amplitudes is 0.62+0.820.62+0.82, which is 0.36+0.64=10.36+0.64=1, as we desired.

For a more general quantum state, the amplitudes can be complex numbers, let’s denote them by αα and ββ so the state is α∣0⟩+β∣1⟩α∣0⟩+β∣1⟩. The constraint is now that the sum of the squares of the amplitudes is 11, i.e., ∣α∣2+∣β∣2=1∣α∣2+∣β∣2=1.

This is called the normalization constraint.

It’s called that because if you think of ∣0⟩∣0⟩ and ∣1⟩∣1⟩ as orthonormal vectors, as I drew them earlier,

then the normalization constraint is the requirement that the length of the state is equal to 11. So it’s a unit vector, or a normalized vector, and that’s why this is called a normalization constraint.
...
One common gotcha in thinking about qubits is to look at the ∣0⟩∣0⟩ state and think it must be the zero vector in the vector space, often denoted 00. But that’s not right at all. The zero vector is at the origin, 0=[00]0=[00], while the ∣0⟩∣0⟩ vector is quite different, ∣0⟩=[10]≠0∣0⟩=[10]=0. It’s just an unfortunate notational accident that ∣0⟩∣0⟩ looks as though it should be the 00 vector. Fortunately, in practice this distinction is easy to get used to, and doesn’t cause confusion. But it’s worth noting.
...
In the case of classical bits, it’s pretty easy to think about the state. You can think of the 00 or 11 states of a bit as corresponding to two very different (but stable) states of a physical system. For instance, a 00 or 11 can be indicated by the presence or absence of a hole at some location in a punch card. And so a single punch card can be used to store hundreds or thousands of bits:

Source: Gwern (2006).

Most ways of storing classical bits are variations on this idea. For instance, the dynamic random access memory (RAM) inside your computer is based on the idea of having two tiny metal plates separated by a miniscule gap. Electric charge is stored on those plates, setting up an electric field between them. The 00 and 11 states of the bit correspond to two different configurations of charge on the plates. In practice, real dynamic RAM systems use slightly more elaborate ideas, but that’s the heart of it. This is harder to think about than punch cards – most of us don’t have so much experience thinking about moving electric charges around metal plates. But it’s still pretty concrete.

So, how should we think about quantum states? The ∣0⟩∣0⟩ and ∣1⟩∣1⟩ states often aren’t difficult to think about, since they often correspond to very concrete physical states of the world, much like classical bits. Indeed, in some proposals they may correspond to different charge configurations, similar to dynamic RAM. Or perhaps a photon being in one of two different locations in space – again, a pretty simple, concrete notion, even if photons aren’t that familiar. There are also many more exotic proposals for the ∣0⟩∣0⟩ and ∣1⟩∣1⟩ states. But by and large you can think of the computational basis states as representing a physical system in one of two well-defined configurations.

Of course, qubits have states which aren’t computational basis states, states like 0.6∣0⟩+0.8∣1⟩0.6∣0⟩+0.8∣1⟩. How should we think about such superposition states? At least in popular media accounts, a very common description is that a state like 0.6∣0⟩+0.8∣1⟩0.6∣0⟩+0.8∣1⟩ is “simultaneously” in the ∣0⟩∣0⟩ state and the ∣1⟩∣1⟩ state.

I must confess, I don’t understand what people mean by this. As far as I can tell, what they’re trying to do is explain the quantum state in terms of classical concepts they’re already familiar with. But I can’t make much sense of it – saying 0.6∣0⟩+0.8∣1⟩0.6∣0⟩+0.8∣1⟩ is simultaneously in the ∣0⟩∣0⟩ state and the ∣1⟩∣1⟩ state seems like word salad, and makes about as much sense to me as Lewis Carroll’s nonsense poem Jabberwocky:
...
Let’s take a look at our very first quantum logic gate, the quantum NOT gate. As you can no doubt surmise, the quantum NOT gate is a generalization of the classical NOT gate. On the computational basis states the quantum NOT gate does just what you’d expect, mimicking the classical NOT gate. That is, it takes the ∣0⟩∣0⟩ state to ∣1⟩∣1⟩, and vice versa:

NOT∣0⟩=∣1⟩NOT∣0⟩=∣1⟩

NOT∣1⟩=∣0⟩NOT∣1⟩=∣0⟩

But the computational basis states aren’t the only states possible for a qubit. What happens when we apply the NOT gate to a general superposition state, that is, α∣0⟩+β∣1⟩α∣0⟩+β∣1⟩? In fact, it does pretty much the simplest possible thing: it acts linearly on the quantum state, interchanging the role of ∣0⟩∣0⟩ and ∣1⟩∣1⟩:

NOT(α∣0⟩+β∣1⟩)=α∣1⟩+β∣0⟩.NOT(α∣0⟩+β∣1⟩)=α∣1⟩+β∣0⟩.

I’ve been using the notation NOT for the quantum NOT gate. But for historical reasons people working on quantum computing usually use a different notation, the notation XX. And so the above may be rewritten:

X(α∣0⟩+β∣1⟩)=α∣1⟩+β∣0⟩.X(α∣0⟩+β∣1⟩)=α∣1⟩+β∣0⟩.

I’ll use the terms XX gate and NOT gate interchangeably.

Historically, the notation XX traces its origin to 1927, when the physicist Wolfgang Pauli introduced an operation sxsx (often written σxσx in textbooks today) to help describe rotations of certain objects around the xx spatial axis. This operation later became of interest to people working on quantum computing. But by that point the ss (and the connection to rotation) was irrelevant, and so sxsx just became XX.

What we’ve seen so far are very algebraic ways of writing down the way the XX gate works. There’s an alternate representation, the quantum circuit representation. In a quantum circuit we depict an XX gate as follows:

The line from left to right is what’s called a quantum wire. A quantum wire represents a single qubit. The term “wire” and the way it’s drawn looks like the qubit is moving through space. But it's often helpful to instead think of left-to-right as representing the passage of time. So the initial segment of wire is just the passage of time, with nothing happening to the qubit. Then the XX gate is applied. And then the quantum wire continues, leaving the desired output.

Sometimes we’ll put the input and output states explicitly in the quantum circuit, so we have something like:

So that’s the quantum circuit representation of the XX gate. It is, in fact, our first quantum computation. A simple computation, involving just a single qubit and a single gate, but a genuine quantum computation nonetheless!

There’s a third representation for the XX gate that’s worth knowing about, a representation as a 2×22×2 matrix:

X=[0110].X=[0110].

To understand in what sense this is a representation of the NOT gate, recall that α∣0⟩+β∣1⟩α∣0⟩+β∣1⟩ is just the vector [αβ][αβ]. And so we have:

[0110](α∣0⟩+β∣1⟩)=[0110][αβ]=[βα]=α∣1⟩+β∣0⟩=X(α∣0⟩+β∣1⟩).[0110](α∣0⟩+β∣1⟩)=[0110][αβ]=[βα]=α∣1⟩+β∣0⟩=X(α∣0⟩+β∣1⟩).

This tells us that XX and [0110][0110] act in exactly the same way on all vectors, and thus are the same operation. In fact, as we’ll see later, it turns out that all quantum gates can be thought of as matrices, with the matrix entries specifying the exact details of the gate.

By the way, regarding the XX gate as a matrix clarifies what may have been a confusing point earlier. I wrote the XX gate as having the action X∣0⟩=∣1⟩X∣0⟩=∣1⟩, X∣1⟩=∣0⟩X∣1⟩=∣0⟩. Implicitly – I never quite said this, though it’s true – the XX gate is a mathematical function, taking input states to output states. But when we write functions we usually use parentheses, so why didn’t I write X(∣0⟩)X(∣0⟩) and similarly for ∣1⟩∣1⟩? The reason is that for linear functions, i.e., matrices, it’s conventional to omit parentheses and just write X∣0⟩X∣0⟩ – function application is just matrix multiplication.
...
This circuit is just a single qubit being preserved in time. To be more explicit, if some arbitrary quantum state ∣ψ⟩∣ψ⟩ is input to the circuit, then the exact same state ∣ψ⟩∣ψ⟩ is output (it’s common practice to use the Greek letter ψψ to denote an arbitrary quantum state):
...
You’ll note that some of the questions above have a different flavor to the questions earlier in the essay. Early questions had cut-and-dried answers. The answer to the question “What’s the standard notation for the quantum NOT gate?” is just: “XX”. But some of the questions above have less well-specified answers. They’re a little fluffy.
...
Let’s take a look at a simple multiple-gate quantum circuit. It’s just a circuit with two XX gates in a row:
...
We’ll try two different ways of figuring out what’s going on. Here’s one approach, based on applying XX twice to an arbitrary input state, α∣0⟩+β∣1⟩α∣0⟩+β∣1⟩. It’s simple algebra, using the fact XX interchanges the ∣0⟩∣0⟩ and ∣1⟩∣1⟩ states:

X(X(α∣0⟩+β∣1⟩))=X(α∣1⟩+β∣0⟩)=α∣0⟩+β∣1⟩X(X(α∣0⟩+β∣1⟩))=X(α∣1⟩+β∣0⟩)=α∣0⟩+β∣1⟩
...
A second way of seeing this is based on the matrix representation we found earlier for the XX gate. Observe that if the input is some arbitrary quantum state ∣ψ⟩∣ψ⟩, then after the first gate the state is X∣ψ⟩X∣ψ⟩, and after the second gate the state is XX∣ψ⟩XX∣ψ⟩. Then we observe that the product XXXX is

XX=[0110][0110]=[1001].XX=[0110][0110]=[1001].

That is, XXXX is the identity operation, and so the output XX∣ψ⟩XX∣ψ⟩ is just the original input ∣ψ⟩∣ψ⟩. In other words, two XX gates is the same as a quantum wire. And so we’ve arrived at the same conclusion in a different way, using matrix multiplication. Doing such matrix multiplications is a pretty common way of analyzing quantum circuits.

## The Hadamard gate

We’ve seen our first quantum gate, the NOT or XX gate. Of course, the XX didn’t appear to do all that much beyond what is possible with a classical NOT gate. In this section I introduce a gate that clearly involves quantum effects, the Hadamard gate.

As with the XX gate, we’ll start by explaining how the Hadamard gate acts on computational basis states. Denoting the gate by HH, here’s what it does:

H∣0⟩=∣0⟩+∣1⟩2H∣0⟩=2∣0⟩+∣1⟩

H∣1⟩=∣0⟩−∣1⟩2H∣1⟩=2∣0⟩−∣1⟩

Of course, ∣0⟩∣0⟩ and ∣1⟩∣1⟩ aren’t the only quantum states. How does the Hadamard gate act on more general quantum states?

It won’t surprise you to learn that it acts linearly, as did the quantum NOT gate. In particular, the Hadamard gate takes a superposition α∣0⟩+β∣1⟩α∣0⟩+β∣1⟩ to the corresponding superposition of outputs:

H(α∣0⟩+β∣1⟩)=α(∣0⟩+∣1⟩2)+β(∣0⟩−∣1⟩2).H(α∣0⟩+β∣1⟩)=α(2∣0⟩+∣1⟩)+β(2∣0⟩−∣1⟩).

That’s a mess. We can make it less of a mess by combining the ∣0⟩∣0⟩ terms together, and also the ∣1⟩∣1⟩ terms together:

H(α∣0⟩+β∣1⟩)=α+β2∣0⟩+α−β2∣1⟩.H(α∣0⟩+β∣1⟩)=2α+β∣0⟩+2α−β∣1⟩.

That’s better, but still not pretty! Fortunately, we mostly won’t be dealing with such complex expressions. The only reason I’ve done it here is to be really explicit. Instead of dealing with such explicit expressions, we’ll mostly work with the HH gate in its circuit and matrix representations (see below). Those let us focus at a more enlightening level of abstraction, rather than messing around with coefficients.
...
Speaking of nuts-and-bolts, let’s get back to the Hadamard gate. Here’s the circuit representation for the Hadamard gate. It looks just like the XX gate in the circuit representation, except we change the gate label to HH:

Just like the XX gate, HH has a matrix representation:

H=12[111−1].H=21[111−1].

To see that this matrix representation is correct, let’s check the action of the matrix on the ∣0⟩∣0⟩ and the ∣1⟩∣1⟩ states. Here we check for the ∣0⟩∣0⟩ state:

12[111−1]∣0⟩=12[111−1][10]=12[11]=∣0⟩+∣1⟩2.21[111−1]∣0⟩=21[111−1][10]=21[11]=2∣0⟩+∣1⟩.

That is, this matrix acts the same way as the Hadamard gate on the ∣0⟩∣0⟩ state. Now let’s check on the ∣1⟩∣1⟩ state:

12[111−1]∣1⟩=12[111−1][01]=12[1−1]=∣0⟩−∣1⟩2.21[111−1]∣1⟩=21[111−1][01]=21[1−1]=2∣0⟩−∣1⟩.

So the matrix acts the same way as the Hadamard gate on both the ∣0⟩∣0⟩ and ∣1⟩∣1⟩ states. By the linearity of matrix multiplication it follows that the matrix acts the same way as the Hadamard on all input states, and so they are the same operation.
...
Here’s one heuristic you can use to think about this circuit: you can think of HH as sort of mixing the ∣0⟩∣0⟩ and ∣1⟩∣1⟩ states together. So if you apply HH twice to ∣0⟩∣0⟩, perhaps it would thoroughly mix the ∣0⟩∣0⟩ and ∣1⟩∣1⟩ up together. What sort of state do you think would result, according to this heuristic? Do you believe the result? Why or why not? Can you think of other heuristics that might help you guess an answer?

Alright, let’s compute what actually happens. After we apply the first Hadamard to ∣0⟩∣0⟩ we get

∣0⟩+∣1⟩2.2∣0⟩+∣1⟩.

Then we apply a second Hadamard gate. This takes the ∣0⟩∣0⟩ term above to ∣0⟩+∣1⟩22∣0⟩+∣1⟩ and the ∣1⟩∣1⟩ term to ∣0⟩−∣1⟩22∣0⟩−∣1⟩, so the output is:

12(∣0⟩+∣1⟩2+∣0⟩−∣1⟩2).21(2∣0⟩+∣1⟩+2∣0⟩−∣1⟩).

If you look at the expression above, you’ll notice that the ∣1⟩∣1⟩ terms cancel each other out, so you’re just left with the ∣0⟩∣0⟩ terms. Collecting them up, we’re left with the ∣0⟩∣0⟩ state, same as we started with:

12(∣0⟩2+∣0⟩2)=∣0⟩.21(2∣0⟩+2∣0⟩)=∣0⟩.

In a similar fashion, after we run the ∣1⟩∣1⟩ state through the first Hadamard gate, we get:
...
This time it’s the ∣0⟩∣0⟩ terms which cancel out, and the ∣1⟩∣1⟩ terms reinforce. When we collect up these terms, we see that the output is just the ∣1⟩∣1⟩ state, same as we started with. And so both the ∣0⟩∣0⟩ and ∣1⟩∣1⟩ states are left unchanged by this quantum circuit, and the net effect of the circuit is exactly the same as a quantum wire:

There’s an alternate way of seeing this, which I’ll sketch out without working through in detail. It’s to note that if we input an arbitrary quantum state ∣ψ⟩∣ψ⟩ to the circuit, then the output must be HH∣ψ⟩HH∣ψ⟩, i.e., the result of applying two HH matrices to ∣ψ⟩∣ψ⟩. But if you just compute the matrix product HHHH it turns out to be the 2×22×2 identity matrix, HH=IHH=I. And so the output from the circuit must be the same as the input, HH∣ψ⟩=I∣ψ⟩=∣ψ⟩HH∣ψ⟩=I∣ψ⟩=∣ψ⟩, just as from a quantum wire.

Of course, this result violates our intuitive guess, which was that two Hadamards would thoroughly mix up the ∣0⟩∣0⟩ and the ∣1⟩∣1⟩. It’s interesting to ponder what went wrong with our intuition, say by looking through the calculation for HH acting twice on ∣0⟩∣0⟩. You see that after the second gate the ∣1⟩∣1⟩ terms exactly cancel one another out, while the ∣0⟩∣0⟩ terms reinforce one another.

This seems innocuous, almost like a mathematical accident. Still, I draw your attention to it because this type of cancellation or reinforcement is crucial in many algorithms for quantum computers. Without getting into details, the rough way many such algorithms work is to first use Hadamard gates to “spread out” in quantum states like ∣0⟩+∣1⟩22∣0⟩+∣1⟩ (or many-qubit analogs), i.e., in superpositions of multiple computational basis states. At the end of the algorithm they use clever patterns of cancellation and reinforcement to bring things back together again into one (or possibly a few, in the many-qubit case) computational basis state, containing the desired answer. That’s a somewhat vague and perhaps tantalizing description, but the point to take away is that the kind of cancellation-or-reinforcement we saw above is actually crucial in many quantum computations.
...
Exercise: Verify that HH=IHH=I, where II is the 2×22×2 identity matrix, I=[1001]I=[1001].

Exercise: Suppose that instead of HH we’d defined a matrix JJ by:

J:=12[1111]J:=21[1111]

At first, it might seem that JJ would make an interesting quantum gate, along lines similar to HH. For instance, J∣0⟩=∣0⟩+∣1⟩2J∣0⟩=2∣0⟩+∣1⟩, and J∣1⟩=∣0⟩+∣1⟩2J∣1⟩=2∣0⟩+∣1⟩. These are both good, normalized quantum states. But what happens if we apply JJ to the quantum state ∣0⟩−∣1⟩22∣0⟩−∣1⟩? Why does this make JJ unsuitable for use as a quantum gate?

Exercise: Consider the quantum circuit:

Explain why the output from this circuit is XH∣ψ⟩XH∣ψ⟩, not HX∣ψ⟩HX∣ψ⟩, as you might naively assume if you wrote down gates in the order they occur in the circuit. This is a common gotcha to be aware of – it occurs because quantum gates compose left-to-right in the circuit representation, while matrix multiplications compose right-to-left.

## Measuring a qubit

Suppose a (hypothetical!) quantum physicist named Alice prepares a qubit in her laboratory, in a quantum state α∣0⟩+β∣1⟩α∣0⟩+β∣1⟩. Then she gives her qubit to another quantum physicist, Bob, but doesn’t tell him the values of αα and ββ. Is there some way Bob can figure out αα and ββ? That is, is there some experiment Bob can do to figure out the identity of the quantum state?

The surprising answer to this question turns out to be NO! There is, in fact, no way to figure out αα and ββ if they start out unknown. To put it a slightly different way, the quantum state of any system – whether it be a qubit or a some other system – is not directly observable.
...
So, what can we figure out from the quantum state? Rather than somehow measuring αα and ββ, there are other ways of getting useful information out of a qubit. Let me describe an especially important process called measurement in the computational basis. This is a fundamental primitive in quantum computing: it’s the way we typically extract information from our quantum computers. I’ll explain now how it works for a single qubit, and later generalize to multi-qubit systems.

Suppose a qubit is in the state α∣0⟩+β∣1⟩α∣0⟩+β∣1⟩. When you measure this qubit in the computational basis it gives you a classical bit of information: it gives you the outcome 00 with probability ∣α∣2∣α∣2, and the outcome 11 with probability ∣β∣2∣β∣2.
...
After the measurement interaction, your measurement apparatus registers an outcome. For instance, it might be that you get the outcome 00. Or maybe instead you get the outcome 11. The crucial fact is that the outcome is ordinary classical information – the stuff you already know how to think about – which you can then use to do other things, and to control other processes.
...
A fundamental fact about this measurement process is that it disturbs the state of the quantum system. In particular, it doesn’t just leave the quantum state alone. After the measurement, if you get the outcome 00 then the state of the qubit afterwards (the “posterior state”) is the computational basis state ∣0⟩∣0⟩. On the other hand, if you get the outcome 11 then the posterior state of the qubit is the computational basis state ∣1⟩∣1⟩.
...
A key point to note is that after the measurement, no matter what the outcome, αα and ββ are gone. No matter whether the posterior state is ∣0⟩∣0⟩ or ∣1⟩∣1⟩, there is no trace of αα or ββ. And so you can’t get any more information about them. In that sense, αα and ββ are a kind of hidden information – the measurement doesn’t tell you what they were.
...
It’s a single-qubit quantum circuit, with input the state ∣ψ⟩∣ψ⟩. A NOT gate is applied, followed by a Hadamard gate. The circuit finishes with a measurement in the computational basis, denoted by the slightly elongated semi-circle. The mm is a classical bit denoting the measurement result – either 00 or 11 – and we use the double wire to indicate the classical bit mm going off and being used to do something else.

Of course, I said above that after measurement the qubit is in either the ∣0⟩∣0⟩ or the ∣1⟩∣1⟩ state. You might think we’d draw a corresponding quantum wire coming out the other side of the measurement. But often in quantum circuits the qubit is discarded after measurement, and that’s assumed by this notation.
...
Then the probability of the two possible measurement outcomes, 00 and 11, must sum to 11, and so we have:

∣α∣2+∣β∣2=1.∣α∣2+∣β∣2=1.

This is exactly the normalization condition for quantum states – i.e., the quantum state must have length 11. The origin of that constraint is really just the fact that measurement probabilities must add up to 11.

Exercise: Suppose we’ve been given either the state ∣0⟩+∣1⟩22∣0⟩+∣1⟩ or the state ∣0⟩−∣1⟩22∣0⟩−∣1⟩, but not told which state we’ve been given. We’d like to figure out which state we’ve been given. If we just do a computational basis measurement, then for both states we get outcome 00 with probability 1221, and outcome 11 with probability 1221. So we can’t distinguish the states directly using a computational basis measurement. But suppose instead we put the state into the following circuit:

Show that if the state ∣0⟩+∣1⟩22∣0⟩+∣1⟩ is input then the output is m=0m=0 with probability 11, while if the state ∣0⟩−∣1⟩22∣0⟩−∣1⟩ is input then the output is m=1m=1 with probability 11. Thus while these two states are indistinguishable if just measured in the computational basis, they can be distinguished with the help of a simple quantum circuit.
...
X=[0110]; H=12[111−1]X=[0110];H=21[111−1]

If the input to these gates is the quantum state ∣ψ⟩∣ψ⟩, then the output is X∣ψ⟩X∣ψ⟩ and H∣ψ⟩H∣ψ⟩ respectively.

A general single-qubit gate works similarly. In particular, a general single-qubit gate can be represented as a 2×22×2 unitary matrix, UU. (If you’re rusty, I’ll remind you what it means for a matrix to be unitary in a moment, and you can just think of it as a matrix for now.) If the input to the gate is the state ∣ψ⟩∣ψ⟩ then the output from the gate is U∣ψ⟩U∣ψ⟩. And so the NOT and Hadamard gates correspond to the special cases where U=XU=X and U=HU=H, respectively.

What does it mean for a matrix UU to be unitary? It’s easiest to answer this question algebraically, where it simply means that U†U=IU†U=I, that is, the adjoint of UU, denoted U†U†, times UU, is equal to the identity matrix. That adjoint is, recall, the complex transpose of UU:

U†:=(UT)∗.U†:=(UT)∗.

So for a 2×22×2 matrix, the adjoint operation is just:

[abcd]†=[a∗c∗b∗d∗].[acbd]†=[a∗b∗c∗d∗].
...
There are a few basic questions you might ask: why are single-qubit gates described by unitary matrices? And how can we get an intuitive feel for what it means for a matrix to be unitary, anyway? While the equation U†U=IU†U=I is easy to check algebraically, we’d like some intuition for what that equation means.
...
Yet another good question is whether there are useful examples of single-qubit gates that aren’t the NOT or Hadamard gates? The equation U†U=IU†U=I is all very well, but it’d be nice to have more concrete examples than just an abstract equation.

We’ll answer all these questions over the next few sections.

Let’s start by checking the unitarity of the Hadamard gate. We start by computing the adjoint of HH:

H†=((12[111−1])T)∗.H†=((21[111−1])T)∗.

Note that taking the transpose doesn’t change the matrix, since it is already symmetric. And taking the complex conjugate doesn’t change anything, since all the entries are real. So we have H†=HH†=H, and thus H†H=HHH†H=HH. But we saw earlier in the essay that HH=IHH=I. So HH is, indeed, unitary.

Exercise: Show that XX is unitary.

Exercise: Show that the identity matrix II is unitary.

Exercise: Can you find an example of a 2×22×2 matrix that is unitary, and is not II, XX, or HH?

For flavor, let’s give a few more examples of single-qubit quantum gates. Earlier in the essay, I mentioned that the NOT gate XX was introduced by the physicist Wolfgang Pauli in the early days of quantum mechanics. He introduced two other matrices, YY and ZZ, which are also useful quantum gates. The three gates, X,YX,Y, and ZZ are known collectively as the Pauli matrices. The YY and ZZ gates will be useful extra tools in our toolkit of quantum gates; in terms of the earlier analogy they expand the repertoire of moves we have available to us. They’re crucial, for example, in protocols such as quantum teleportation and quantum error correction.

The YY gate is similar to the XX gate, but instead of 11s on the off-diagonal, it has ii and −i−i, so it takes ∣0⟩∣0⟩ to i∣1⟩i∣1⟩ and ∣1⟩∣1⟩ to −i∣0⟩−i∣0⟩:

Y:=[0−ii0].Y:=[0i−i0].

The ZZ gate leaves ∣0⟩∣0⟩ unchanged, and takes ∣1⟩∣1⟩ to −∣1⟩−∣1⟩:

Z:=[100−1].Z:=[100−1].

Exercise: Show that the YY and ZZ matrices are unitary, and so legitimate quantum gates.

Another good example of a quantum gate is a rotation, the kind of matrix you’ve most likely been seeing since high school. It’s just the ordinary rotation of the 22-dimensional plane by an angle θθ:
...
Can we get an intuition for what it means for a matrix to be unitary? It turns out that unitary matrices preserve the length of their inputs. In other words, if we take any vector ∣ψ⟩∣ψ⟩ and compute the length ∥U∣ψ⟩∥∥U∣ψ⟩∥ it’s always equal to the length ∥∣ψ⟩∥∥∣ψ⟩∥ of the original vector. In this, they’re much like rotations or reflections in ordinary (real) space, which also don’t change lengths. In a sense, the unitary matrices are a complex generalization of real rotations and reflections.

Up to now we’ve been dealing mostly with vectors which are states of qubits, i.e., normalized 2-dimensional vectors. But in fact the statement of the last paragraph is true for d×dd×d unitary matrices, too, i.e., ∥U∣ψ⟩∥=∥∣ψ⟩∥∥U∣ψ⟩∥=∥∣ψ⟩∥ is true for any vector ∣ψ⟩∣ψ⟩ in dd dimensions.

The proof is actually pretty simple – mostly, we just compute ∥U∣ψ⟩∥∥U∣ψ⟩∥ and use a little algebra to check that it’s equal to the length ∥∣ψ⟩∥∥∣ψ⟩∥. But before we get to the proof, observe that this result is good news for quantum gates. The reason is that in a quantum gate the input state will be normalized (have length 11), since that’s a requirement for a quantum state. And we’d expect the corresponding output state to also be normalized, otherwise it wouldn’t be a legitimate quantum state. Fortunately, the length-preserving property of unitary matrices ensures the output state is properly normalized.

Recall also the larger story this is part of: not only are gates unitary, to ensure that states remain normalized; also, quantum states are normalized, since measurement probabilities must sum to 11. It all fits together.

In fact it turns out that unitary matrices are the only matrices which preserve length in this way. And so a good way of thinking about unitary matrices is that they’re exactly the class of matrices which preserve length. That’s the geometric interpretation (and intuitive meaning) of the algebraic condition U†U=IU†U=I.

Let’s prove now that unitary matrices really are length-preserving.

The proof is straightforward. The main thing is to compute ∥U∣ψ⟩∥∥U∣ψ⟩∥. Actually, it’s a little easier to compute the square of the length, ∥U∣ψ⟩∥2∥U∣ψ⟩∥2. This is just the sum of the squares of the absolute values of the components of the vector U∣ψ⟩U∣ψ⟩:

∥U∣ψ⟩∥2=∑j(Uψ)j∗(Uψ)j∥U∣ψ⟩∥2=j∑(Uψ)j∗(Uψ)j

Note that I’ve used (Uψ)j(Uψ)j to denote the jjth component of U∣ψ⟩U∣ψ⟩, dropping the full ket notation, and just using ψψ alone. To proceed, we’ll expand the equation above out, and look for opportunities to apply the unitarity of UU. In particular, the component (Uψ)j(Uψ)j is given by ∑lUjlψl∑lUjlψl and similarly for the complex conjugate term. So we can rewrite the above equation as:

∥U∣ψ⟩∥2=∑jklUjk∗ψk∗Ujlψl∥U∣ψ⟩∥2=jkl∑Ujk∗ψk∗Ujlψl

To make use of the unitarity of UU we’ll move the UU terms together, and rewrite in terms of U†U†. This gives

∥U∣ψ⟩∥2=∑jklUkj†Ujlψk∗ψl,∥U∣ψ⟩∥2=jkl∑Ukj†Ujlψk∗ψl,

where we interchanged the jkjk indices on the first UU in order to rewrite it in terms of U†U†. The only place the jj index appears anywhere in this equation is in the Ukj†UjlUkj†Ujl term. We can therefore perform the sum over jj and those terms become (U†U)kl(U†U)kl, which is just the klklth term in the identity matrix, since U†U=IU†U=I. So the above equation becomes:

∥U∣ψ⟩∥2=∑klδklψk∗ψl.∥U∣ψ⟩∥2=kl∑δklψk∗ψl.

When we sum, the only time δklδkl is not equal to zero is when k=lk=l, in which case it’s 11. And so we can get rid of one of the summation indices, and the equation simplifies to:

∥U∣ψ⟩∥2=∑kψk∗ψk.∥U∣ψ⟩∥2=k∑ψk∗ψk.

The right-hand side is, of course, equal to the norm of ∣ψ⟩∣ψ⟩ squared. And so we’ve shown ∥U∣ψ⟩∥2=∥∣ψ⟩∥2∥U∣ψ⟩∥2=∥∣ψ⟩∥2, and therefore ∥U∣ψ⟩∥=∥∣ψ⟩∥∥U∣ψ⟩∥=∥∣ψ⟩∥. That completes the proof that unitary matrices are length-preserving. QED
...
Theorem: Let MM be a matrix. Then ∥M∣ψ⟩∥=∥∣ψ⟩∥∥M∣ψ⟩∥=∥∣ψ⟩∥ for all vectors ∣ψ⟩∣ψ⟩ if and only if MM is unitary.
...
We’ve been dealing with 2-dimensional vectors up to now, but what I’m about to say applies no matter how many dimensions we’re working in. So suppose we have a vector ∣ψ⟩∣ψ⟩ which can be written in component form as:

∣ψ⟩=[ab⋮z].∣ψ⟩=⎣⎢⎢⎢⎡ab⋮z⎦⎥⎥⎥⎤.

We’re going to define a new object, also labeled with a ψψ, but now with a bracket in the other direction:

⟨ψ∣:=[a∗b∗…z∗].⟨ψ∣:=[a∗b∗…z∗].

That is, ⟨ψ∣⟨ψ∣ is a row vector, whose entries are the same as ∣ψ⟩∣ψ⟩, but complex conjugated. The vector ∣ψ⟩∣ψ⟩ was called a ket, and (you’re going to groan) ⟨ψ∣⟨ψ∣ is called a bra, making this the bra-ket, or bracket notation. Yes, theoretical physicists make dad jokes, too. These names were given by the theoretical physicist Paul Dirac in 1939, and it’s often called the Dirac bra-ket notation, or sometimes just the Dirac notation.

A key fact about the bra ⟨ψ∣⟨ψ∣ is that it’s related to the ket ∣ψ⟩∣ψ⟩ by the dagger operation:
...
and apply the dagger operation, which means taking the transpose, turning it into a row vector with entries a,b,…,za,b,…,z, and then take the complex conjugate, giving us [a∗b∗…z∗][a∗b∗…z∗], which is just the definition of ⟨ψ∣⟨ψ∣.

In a similar way we see that ⟨ψ∣†=∣ψ⟩⟨ψ∣†=∣ψ⟩.

Another useful identity expresses the length of ∣ψ⟩∣ψ⟩ in terms of the Dirac notation:

∥∣ψ⟩∥2=⟨ψ∣∣ψ⟩.∥∣ψ⟩∥2=⟨ψ∣∣ψ⟩.

That is, the length squared is just equal to the product of the row vector ⟨ψ∣⟨ψ∣ with the column vector ∣ψ⟩∣ψ⟩. To prove it just notice that the right-hand side is

[a∗b∗…][ab⋮]=∣a∣2+∣b∣2+…[a∗b∗…]⎣⎢⎡ab⋮⎦⎥⎤=∣a∣2+∣b∣2+…

And that, of course, is ∥∣ψ⟩∥2∥∣ψ⟩∥2, just as we wanted.

Physicists using the Dirac notation don’t usually write ⟨ψ∣∣ψ⟩⟨ψ∣∣ψ⟩. They simplify it slightly, omitting one of the vertical bars ∣∣ in the middle, and just write it as:

⟨ψ∣ψ⟩.⟨ψ∣ψ⟩.

It’s only a slight simplification, but this omission of the extra bar turns out to make life considerably easier, and is well worth it. I’ve shown it above just for the special case of ⟨ψ∣ψ⟩⟨ψ∣ψ⟩ but the same omission of a vertical bar is done often in other contexts. In practice, it rarely causes confusion, although of course you do need to get used to it.

Another useful identity is that if MM is a matrix and ∣ψ⟩∣ψ⟩ is a ket, then

(M∣ψ⟩)†=⟨ψ∣M†.(M∣ψ⟩)†=⟨ψ∣M†.

If you think of ⟨ψ∣⟨ψ∣ as the dagger of ∣ψ⟩∣ψ⟩, then what this equation means is that taking the dagger of the product M∣ψ⟩M∣ψ⟩ is the same as taking the product of the dagger of MM with the dagger ∣ψ⟩∣ψ⟩, but reversing the order. This is a useful little mnemonic for remembering the above identity.
...
Proof: The way to prove the identity is to apply the definitions. We’re going to look at the jjth component of the left-hand side, (M∣ψ⟩)j†(M∣ψ⟩)j†, and we’ll show it’s equal to the jjth component of the right-hand side. By definition, the jjth row component (M∣ψ⟩)j†(M∣ψ⟩)j† is equal to the complex conjugate of the jjth column component of M∣ψ⟩M∣ψ⟩, i.e., (M∣ψ⟩)j∗(M∣ψ⟩)j∗. That column component is ∑kMjk∗ψk∗∑kMjk∗ψk∗. We can move the ψψ terms to the left, and swap the indices on the MM term to convert the ∗∗ to a dagger, giving ∑kψk∗Mkj†∑kψk∗Mkj†. That’s just the jjth component of the row vector ⟨ψ∣M†⟨ψ∣M†, as we set out to show. QED

With all these ideas in mind, here’s an exercise for you to work through, putting several of these ideas together:

Exercise: Show that for any matrix MM and vector ∣ψ⟩∣ψ⟩, the following identity holds, expressing the length of M∣ψ⟩M∣ψ⟩:

∥M∣ψ⟩∥2=⟨ψ∣M†M∣ψ⟩.∥M∣ψ⟩∥2=⟨ψ∣M†M∣ψ⟩.
...
Alright, we’ve been working through some heavy, detailed material. We’ve got just a little more background to get through, but it’s easier going. I’m going to introduce a unit vector, denoted ∣ej⟩∣ej⟩, meaning the vector with a 11 in the jjth component, and 00s everywhere else. So, for instance, for a qubit:

∣e0⟩=[10]∣e1⟩=[01]∣e0⟩∣e1⟩=[10]=[01]

From elementary linear algebra, if MM is a matrix, then M∣ek⟩M∣ek⟩ is just the kkth column of MM. (If you don’t recall that from elementary linear algebra, I encourage you to stop and figure out why it’s true.) And from that you can deduce easily that ⟨ej∣M∣ek⟩⟨ej∣M∣ek⟩ is the jkjkth entry of MM.

Exercise: If you’re not familiar with the proof, show that M∣ek⟩M∣ek⟩ is the kkth column of the matrix MM, and that ⟨ej∣M∣ek⟩⟨ej∣M∣ek⟩ is the jkjkth entry of MM.
...
Proof: We’ll assume MM is length-preserving, and analyze the matrix M†MM†M. Our goal is to show that this is the identity matrix, and thus MM is unitary. To do this, we’re going to start by analyzing the diagonal elements (M†M)jj(M†M)jj, and show that they’re all equal to 11. Then we’ll turn our attention to the off-diagonal elements and show that they’re all equal to 00.

To understand the diagonal elements, we know from earlier that: (M†M)jj=⟨ej∣M†M∣ej⟩=∥M∣ej⟩∥2(M†M)jj=⟨ej∣M†M∣ej⟩=∥M∣ej⟩∥2. But since MM is length-preserving, the latter is just ∥∣ej⟩∥2∥∣ej⟩∥2, which is 11. And so we conclude that all the diagonal elements are, indeed, 11.

What about the off-diagonal elements, i.e., (M†M)jk(M†M)jk where j≠kj=k? Can we show that these are all equal to 00? Well, what we’d like to do is somehow to relate (M†M)jk(M†M)jk to the length of some vector M∣ψ⟩M∣ψ⟩, and then use the length-preserving property. One idea is to try using ∣ψ⟩=∣ej⟩+∣ek⟩∣ψ⟩=∣ej⟩+∣ek⟩, since that involves both the jjth and kkth directions. From the length-preserving property we have:

∥M∣ψ⟩∥2=∥∣ψ⟩∥2=12+12=2.∥M∣ψ⟩∥2=∥∣ψ⟩∥2=12+12=2.

We also have:

∥M∣ψ⟩∥2=⟨ψ∣M†M∣ψ⟩=⟨ej∣M†M∣ej⟩+⟨ej∣M†M∣ek⟩+⟨ek∣M†M∣ej⟩+⟨ek∣M†M∣ek⟩=1+⟨ej∣M†M∣ek⟩+⟨ek∣M†M∣ej⟩+1∥M∣ψ⟩∥2=⟨ψ∣M†M∣ψ⟩=⟨ej∣M†M∣ej⟩+⟨ej∣M†M∣ek⟩+⟨ek∣M†M∣ej⟩+⟨ek∣M†M∣ek⟩=1+⟨ej∣M†M∣ek⟩+⟨ek∣M†M∣ej⟩+1

Comparing the last two sets of equations, we have:

⟨ej∣M†M∣ek⟩+⟨ek∣M†M∣ej⟩=0.⟨ej∣M†M∣ek⟩+⟨ek∣M†M∣ej⟩=0.

This is close to what we want, but isn’t quite right. It tells us that (M†M)jk+(M†M)kj=0(M†M)jk+(M†M)kj=0. Can we do better? It’s tempting to go back and fiddle around and try to find some way of eliminating one or the other of those terms. But there’s no direct way to do it – at least, no direct way that I know of.

But what if we’d done something slightly different, and instead of using ∣ψ⟩=∣ej⟩+∣ek⟩∣ψ⟩=∣ej⟩+∣ek⟩ we’d used ∣ψ⟩=∣ej⟩+i∣ek⟩∣ψ⟩=∣ej⟩+i∣ek⟩? It seems pretty plausible that following the same line of reasoning we’d get an equation involving (M†M)jk(M†M)jk and (M†M)kj(M†M)kj again. I won’t explicitly go through the steps – you can do that yourself – but if you do go through them you end up with the equation:

(M†M)jk−(M†M)kj=0.(M†M)jk−(M†M)kj=0.

This is great! We can add it to the earlier equation to deduce that (M†M)jk=0(M†M)jk=0 whenever j≠kj=k, and so we conclude that M†M=IM†M=I, i.e., MM is unitary. QED
...
Up to now I haven’t said what the possible states of a two-qubit system are, but you can probably guess. We now have four computational basis states, corresponding to the four possible states of a two-bit system: ∣00⟩,∣01⟩,∣10⟩∣00⟩,∣01⟩,∣10⟩, and ∣11⟩∣11⟩. And, for a two-qubit system, not only can we have those four states, we can also have superpositions (i.e., linear combinations) of them:

α∣00⟩+β∣01⟩+γ∣10⟩+δ∣11⟩α∣00⟩+β∣01⟩+γ∣10⟩+δ∣11⟩

Here, the amplitudes α,β,γ,δα,β,γ,δ are just complex numbers, and the sum of the squares of the absolute values is 11, i.e, ∣α∣2+∣β∣2+∣γ∣2+∣δ∣2=1∣α∣2+∣β∣2+∣γ∣2+∣δ∣2=1. This is the same kind of normalization condition as we had for a single qubit.

Now, the CNOT gate is, much like the quantum NOT gate, inspired directly by a classical gate. What it does is very simple. If the control qubit is set to 11, as in the states ∣10⟩∣10⟩ and ∣11⟩∣11⟩, then it flips (i.e., NOTs) the target qubit. And otherwise it does nothing. Writing out the action on all four computational basis states we have:

∣00⟩→∣00⟩∣01⟩→∣01⟩∣10⟩→∣11⟩∣11⟩→∣10⟩∣00⟩∣01⟩∣10⟩∣11⟩→→→→∣00⟩∣01⟩∣11⟩∣10⟩
...
There’s a way of summing up all four of the equations above in a single equation. Suppose xx and yy are classical bits, i.e., 00 or 11. Then we can rewrite the equations above in a single equation as:

∣x,y⟩→∣x,y⊕x⟩.∣x,y⟩→∣x,y⊕x⟩.

Note the commas inserted to make this easier to read – this is pretty common in working with multi-qubit states.

The above equation makes clear that the CNOT leaves the control qubit xx alone, but flips the target qubit yy if xx is set to 11. Note that ⊕⊕ is addition modulo 22, where 1⊕1=01⊕1=0, as we would expect from the fact that the CNOT takes ∣11⟩∣11⟩ to ∣10⟩∣10⟩.
...
α∣00⟩+β∣01⟩+γ∣10⟩+δ∣11⟩→α∣00⟩+β∣01⟩+γ∣11⟩+δ∣10⟩α∣00⟩+β∣01⟩+γ∣10⟩+δ∣11⟩→α∣00⟩+β∣01⟩+γ∣11⟩+δ∣10⟩
...
Of course, the CNOT doesn’t just appear in two-qubit computations. It also appears in computations involving more qubits. Let’s suppose we have three qubits, for instance, and computational basis states such as ∣000⟩,∣001⟩∣000⟩,∣001⟩, and so on. Here’s a CNOT with the second qubit as the control qubit and the third qubit as the target:

What goes on? Well, we can write out what happens on an arbitrary computational basis state, ∣x,y,z⟩∣x,y,z⟩, where x,yx,y and zz are all classical bits. Of course, the first bit xx isn’t changed at all, since it’s not involved in the CNOT. The second bit yy is the control bit, and so isn’t changed. But the third bit zz is flipped if the control bit yy is set to 11. And so we can write the action of the CNOT as:

∣x,y,z⟩→∣x,y,z⊕y⟩∣x,y,z⟩→∣x,y,z⊕y⟩

I’ve described the CNOT as a “classical” gate, but it can be combined with single-qubit gates to do non-classical things. Let me give you an explicit example. It’s another two-qubit computation. It starts with the ∣00⟩∣00⟩ computational basis state, we apply a Hadamard gate to the first qubit, and then do a CNOT:

Recall that for a single qubit the Hadamard gate takes ∣0⟩∣0⟩ to an equal superposition (∣0⟩+∣1⟩)/2(∣0⟩+∣1⟩)/2. For these two qubits it doesn’t affect the second qubit at all, and so it takes ∣00⟩∣00⟩ to (∣00⟩+∣10⟩)/2(∣00⟩+∣10⟩)/2.

Next we apply the CNOT gate. This leaves the ∣00⟩∣00⟩ state unchanged, since the control bit is 00. And it takes ∣10⟩∣10⟩ to ∣11⟩∣11⟩, since the control bit is 11. And so the output from the circuit is:

∣00⟩+∣11⟩2.2∣00⟩+∣11⟩.

This output state is a highly non-classical state – it’s actually a type of state called an entangled state. There’s no obvious interpretation of this state as a classical state, unlike say a computational basis state such as ∣00⟩∣00⟩. In fact, entangled states can be used to do all sorts of interesting information processing tasks, including quantum teleportation and fast quantum algorithms.

A point I glossed over above, but worth mentioning: in the circuit I drew ∣0⟩∣0⟩ and ∣0⟩∣0⟩ separately as input qubits. It’s conventional to do that kind of thing to denote a combined input of ∣00⟩∣00⟩. More generally, people use ∣0⟩∣0⟩∣0⟩∣0⟩ interchangeably with ∣00⟩∣00⟩, ∣0⟩∣1⟩∣0⟩∣1⟩ interchangeably with ∣01⟩∣01⟩, and so on. Going back and forth takes a bit of getting used to, but everything works pretty much as you expect, and you just need a little practice before it seems quite natural.

More generally, if we have single-qubit states α∣0⟩+β∣1⟩α∣0⟩+β∣1⟩ and γ∣0⟩+δ∣1⟩γ∣0⟩+δ∣1⟩, then the combined state when the two qubits are put together is just:

(α∣0⟩+β∣1⟩)(γ∣0⟩+δ∣1⟩)=αγ∣00⟩+αδ∣01⟩+βγ∣10⟩+βδ∣11⟩.(α∣0⟩+β∣1⟩)(γ∣0⟩+δ∣1⟩)=αγ∣00⟩+αδ∣01⟩+βγ∣10⟩+βδ∣11⟩.
...
Exercise: Can you find single-qubit states ∣a⟩∣a⟩ and ∣b⟩∣b⟩ so that applying a CNOT to the combined state ∣ab⟩∣ab⟩ changes the first qubit, i.e., the control qubit?
...
A mnemonic for this notation is that these are both equal superpositions of ∣0⟩∣0⟩ and ∣1⟩∣1⟩, but the “+” or “-” corresponds to the sign of the amplitude for the ∣1⟩∣1⟩ state. The following circuit identity holds:
...
Exercise: We saw earlier that composition of quantum gates corresponds to matrix multiplication (in reverse order). Show that the product of two unitary matrices UU and VV is also unitary. As a consequence, the net effect of any quantum circuit (before measurement) is to effect a unitary operation on the state space of the system.

Let me conclude this section with a brief comment about a particular class of quantum gates. These are gates which are multiples of the identity matrix, II:

[eiθ00eiθ]=eiθ[1001]=eiθI.[eiθ00eiθ]=eiθ[1001]=eiθI.

When θθ is a real number this is a unitary matrix, and so a valid quantum gate. The effect of the gate is simply to multiply the state of the quantum computer by eiθeiθ. The number eiθeiθ is called a global phase factor. But even though the gate is valid, we rarely explicitly put such gates into quantum circuits. The reason is that such global phase factors have no impact on the results of the computation. To see why, imagine a quantum circuit which includes many such quantum gates, possibly for different values, θ1,θ2,…θ1,θ2,…. No matter where in the circuit the gates occur, the net effect is to multiply the state output from the circuit by an overall factor of eiθ1+iθ2+…eiθ1+iθ2+…. This doesn’t change the squared amplitudes of the computational basis states, and so has no impact on the measurement probabilities at the end of the computation. We could have left the gates out and it would make no difference.

Quantum computing people tend to be rather blase about such global phase factors. They’ll do things like not bother to distinguish between unitary gates such as XX and −X−X, saying these gates are “the same up to a global phase factor”. They simply mean the −X−X gate is the same as doing the XX gate, followed by the −I−I gate. Since the latter gate makes no difference to the output from the computation, it can safely be omitted. We’ll see examples in the next essay, about the quantum search algorithm, where at a couple of places it makes sense to multiply the quantum state by a global phase factor of −1−1.
...
This is easy to do with the NOT gate – we just turn it into an XX gate. But what’s not so easy is the AND gate. You might wonder if there’s some quantum gate which takes two bits xx and yy as input, in the computational basis, ∣x,y⟩∣x,y⟩, and then outputs a single qubit ∣x∧y⟩∣x∧y⟩, where x∧yx∧y is just the logical AND of the bits xx and yy.
...
You might wonder if instead there’s some way we can find a two-qubit quantum gate which has x∧yx∧y as one output, and something else as the other output. I won’t prove it, but it turns out that this is impossible. The proof actually isn’t all that hard – it’s a fun exercise to think through, if you want a challenge – but is more of a digression than I want to get into here.

That’s all rather disappointing. But there is a solution. It’s to use a three-qubit quantum gate called the Toffoli gate. The Toffoli gate is much like the CNOT gate, but instead of having a single control qubit, it has two control qubits, xx and yy, and a single target qubit, zz. If both controls qubits are set, then the target is flipped. Otherwise, the target is left alone:

If the target starts out as z=0z=0, then you can see that the target output is just x∧yx∧y, and so the Toffoli gate can be used to simulate a classical AND gate. So if we have any classical circuit of AND and NOT gates then there’s a corresponding quantum circuit involving the same number of XX and Toffoli gates which computes the same function.

Exercise: What’s a quantum circuit that can compute the NAND gate? Recall that the NAND of two bits xx and yy is just the NOT of x∧yx∧y.
...
The reason classical computers are bad at simulating quantum systems isn’t difficult to understand. Suppose we have a molecule containing nn atoms – for a small molecule, nn may be 11-1010, for a complex molecule it may be hundreds or thousands or even more. And suppose we think of each atom as a qubit (not true, but go with it): to describe the system we’d need 2n2n different amplitudes, one amplitude for each nn-bit computational basis state, e.g., ∣010011…⟩∣010011…⟩.

Of course, atoms aren’t qubits. They’re more complicated, and we need more amplitudes to describe them. Without getting into details, the rough scaling for an nn-atom molecule is that we need knkn amplitudes, where k≥2k≥2. The value of kk depends upon context – which aspects of the atom’s behavior are important. For generic quantum simulations kk may be in the hundreds or more.

That’s a lot of amplitudes! Even for comparatively simple atoms and small values of nn, it means the number of amplitudes will be in the trillions. And it rises very rapidly, doubling or more for each extra atom. If k=100k=100, then even n=10n=10 atoms will require 100 million trillion amplitudes. That’s a lot of amplitudes for a pretty simple molecule.
...
> The quantum corollary to Moore’s law:
