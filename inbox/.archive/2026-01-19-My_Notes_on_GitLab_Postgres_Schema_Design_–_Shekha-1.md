...
tags: ['Running']
created: 2024-02-18T07:14:00.000Z
---

# My Notes on GitLab Postgres Schema Design – Shekhar Gulati

## AI Summary (from Notion)
- Purpose of the Document: The author, Shekhar Gulati, shares insights from analyzing the Postgres schema of GitLab, aiming to learn best practices for schema design.

- Primary Key Types:
- GitLab primarily uses bigserial and serial for primary keys, avoiding UUIDs due to performance concerns.
- The choice of primary key type impacts storage space and read/write speeds.

- Internal vs. External IDs:
- GitLab employs both internal (id) and external (iid) identifiers to prevent users from easily guessing the number of records (e.g., issues).

- Character Types:
- The schema favors text over varchar(n) for variable strings, leveraging CHECK constraints for length validation and schema flexibility.

- Naming Conventions:
- Consistent naming practices include plural table names, snake_case for columns, and specific patterns for boolean and index naming.

- Timestamp Usage:
- Differentiates between timestamp with timezone for user actions and timestamp without timezone for system actions.

- Foreign Key Constraints:
- GitLab uses foreign key constraints in most tables to maintain referential integrity, but avoids them in immutable tables for performance reasons.

- Partitioning:
- Utilizes partitioning strategies (RANGE, LIST, HASH) to manage large tables and improve query performance.

- GIN Indexes for Search:
- Implements GIN indexes with the pg_trgm extension for efficient LIKE searches, enhancing performance for text-based queries.

- Usage of JSONB:
- Adopts jsonb data type for flexible schema designs, especially for storing dynamic or unstructured data.

- Additional Insights:
- Favoring inet data types for IP addresses allows for better data integrity checks.
- Emphasis on performance implications of design choices, especially with large datasets.
- Highlights the balance between flexible design and performance optimization in schema development.

- Conclusion: The document encapsulates key takeaways from GitLab's schema design, advocating for context-dependent best practices rather than one-size-fits-all solutions.
