...
tags: ['Running']
created: 2024-06-23T01:47:00.000Z
---

# GitHub - awslabs/aws-lambda-web-adapter: Run web applications on AWS Lambda

## AI Summary (from Notion)
- Project Overview: The AWS Lambda Web Adapter enables developers to run web applications on AWS Lambda using various frameworks like Express.js, Next.js, Flask, and more.
- Key Features:
- Supports multiple web frameworks and languages without adding new dependencies.
- Works with Lambda managed runtimes, custom runtimes, and Docker OCI images.
- Provides automatic binary response encoding, graceful shutdown, response payload compression, and response streaming.
- Compatible with both HTTP and non-HTTP event triggers.
- Usage:
- Can be packaged as Docker images or Zip packages.
- Detailed instructions for integrating the adapter with existing web applications.
- Environment Variables: A range of customizable environment variables allow for configuration of traffic ports, readiness checks, and response handling.
- Readiness Check: The adapter performs a readiness check to ensure the web application is ready to handle requests before starting the Lambda Runtime.
- Local Debugging: Developers can test applications locally using AWS SAM CLI.
- Acknowledgement: The project draws inspiration from various community projects and similar capabilities offered by other frameworks.
