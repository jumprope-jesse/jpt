...
tags: ['Running']
created: 2025-11-30T23:28:00.000Z
---

# Writing a good CLAUDE.md | HumanLayer Blog

## Overview (from Notion)
- The concept of CLAUDE.md emphasizes the importance of clear communication and structured information, which can resonate with your role as a founder, ensuring your team is aligned and informed.
- Leveraging AI in software development can enhance productivity, allowing you to spend more quality time with your family while maintaining project efficiency.
- The idea of stateless LLMs can relate to parenting—each interaction is unique, just like each coding session with an AI, emphasizing the need for adaptability.
- Progressive Disclosure can parallel how you manage family responsibilities, introducing tasks or information at the right time to avoid overwhelming your kids.
- Utilizing tools and frameworks effectively is akin to building a supportive environment for your family, ensuring everyone has the resources they need to succeed.
- Think about how these AI tools could help you mentor younger developers, sharing insights from your experience while allowing them to explore solutions independently.
- Alternate views could emphasize the importance of human intuition and emotional intelligence in both parenting and leadership, reminding you that while AI aids decision-making, human touch remains irreplaceable.

## AI Summary (from Notion)
CLAUDE.md is essential for onboarding Claude to your codebase, detailing the project's purpose, structure, and operational guidelines. It should contain minimal, universally applicable instructions to prevent Claude from ignoring important context. Progressive Disclosure is recommended to manage task-specific instructions separately, and linters should be used instead of relying on Claude for style guidelines. Careful crafting of CLAUDE.md is crucial as it significantly impacts workflow and outcomes.
...
1. The agent must be told anything that's important to know about your codebase each time you start a session.
1. CLAUDE.md is the preferred way of doing this.
...
1. you have a good reason to do so
...
1. Smaller models get MUCH worse, MUCH more quickly. Specifically, smaller models tend to exhibit an expotential decay in instruction-following performance as the number of instructions increase, whereas larger frontier thinking models exhibit a linear decay (see below). For this reason, we recommend against using smaller models for multi-step tasks or complicated implementation plans.
1. LLMs bias towards instructions that are on the peripheries of the prompt: at the very beginning (the Claude Code system message and CLAUDE.md), and at the very end (the most-recent user messages)
1. As instruction count increases, instruction-following quality decreases uniformly. This means that as you give the LLM more instructions, it doesn't simply ignore the newer ("further down in the file") instructions - it begins to ignore all of them uniformly
...
1. Less (instructions) is more. While you shouldn't omit necessary instructions, you should include as few instructions as reasonably possible in the file.
1. Keep the contents of your CLAUDE.md concise and universally applicable.
1. Use Progressive Disclosure - don't tell Claude all the information you could possibly want it to know. Rather, tell it how to find important information so that it can find and use it, but only when it needs to to avoid bloating your context window or instruction count.
1. Claude is not a linter. Use linters and code formatters, and use other features like Hooks and Slash Commands as necessary.
1. CLAUDE.md is the highest leverage point of the harness, so avoid auto-generating it. You should carefully craft its contents for best results.
