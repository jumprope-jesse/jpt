...
tags: ['Running']
created: 2025-06-11T15:11:00.000Z
---

# crawshaw - 2025-06-08

## Overview (from Notion)
- Embrace the shift: The integration of agents in programming can streamline your workflow, allowing you to focus on higher-level problem-solving rather than mundane tasks.
- Balance family and work: With agents handling repetitive programming tasks, you might find more time for family, creating a better work-life balance.
- Stay curious: The rapid evolution of LLM technology can inspire you to continuously learn and adapt, fostering a mindset of growth for both personal and professional development.
- Unique perspective: The concept of agents as feedback-driven tools can transform how you approach coding, making programming feel less isolating by providing interactive support.
- Consider the challenges: While agents can enhance productivity, they may also introduce new complexities, necessitating a reevaluation of traditional coding practices and collaboration methods.
- Alternate views: Some may argue that reliance on agents could dilute coding skills or lead to security vulnerabilities. Balancing agent use with hands-on coding will be crucial in maintaining a robust skill set.

## AI Summary (from Notion)
The text discusses the evolution of programming with agents that utilize large language models (LLMs) to enhance coding efficiency. It defines agents as simple constructs that can execute commands and provide feedback, improving the programming process by allowing LLMs to interact with compilers and navigate codebases. The author shares personal experiences with using agents for tasks like implementing GitHub App authentication and managing SQL conventions, highlighting both their potential and limitations. The future of programming tools is explored, emphasizing the need for new workflows and the integration of agents into development environments.
...
UserLLMpromptbash, patch,etctool calltool resultResponse &End of turn
...
Today it may also appear costly (my last significant agent-driven commit cost me $1.15 in API credits!) but the cost will rapidly disappear as the chips driving the models continue to improve. GPUs are not nearly as restricted in their future progress as CPUs which are far more constrained in how they can physically improve (compiler-driven ILP only goes so far). The fact that we still call LLM chips “graphics” chips shows the economic machine underneath software has a lot of restructuring to do before it is fully focused on LLM performance-per-watt.
...
This post also appears on the sketch blog.

Thanks to Sean McCullough, Philip Zeyliger, and Thomas Ptacek for reviewing this post.
