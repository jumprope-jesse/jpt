...
type: link
source: notion
url: https://rednafi.com/misc/on_rebasing/
notion_type: Tech Deep Dive
tags: ['Running']
created: 2024-06-23T01:54:00.000Z
---

# I kind of like rebasing | Redowan's Reflections

## AI Summary (from Notion)
- Rebasing in Git: The author prefers rebasing over merging due to its ability to maintain a linear history and clean up messy commits before peer review.

- Assumptions:
- The document operates under a typical web service development model.
- Feature branches are forked from a protected main branch.
- Pull requests (PRs) are created for merging changes into the main branch.

- Rebasing Steps:
- Pull the latest changes from the main branch.
- Create a new feature branch.
- Perform interactive rebasing before sending a PR to tidy up commits.
- Resolve any merge conflicts that may arise during rebasing.

- Interactive Rebasing:
- Allows squashing of multiple messy commits into one with a proper commit message.
- The author describes a step-by-step process for squashing commits and managing commit messages during interactive rebasing.

- Common Commands:
- git rebase -i HEAD~n: Initiates interactive rebasing for the last n commits.
- git log main..@ --oneline --graph: Displays commits on the feature branch not present on the main branch.

- Advanced Techniques:
- Splitting a single commit into multiple commits is possible but rarely used.
- Executing commands during interactive rebasing (e.g., running tests) can enhance the process by ensuring tests pass at each commit.

- Key Takeaway: The author emphasizes the importance of maintaining a clean commit history and demonstrates how rebasing can facilitate this, along with practical commands and tips for users.
...
1. Fork off a new branch via git switch -c feat_branch.
1. Do the work in feat_branch, and before sending the PR, do interactive rebasing if necessary, and then rebase the feat_branch onto the latest changes of main with:
1. Push the changes to the remote repository with git push origin HEAD and send a PR against main for review.
...
1. Atomic commits ↩︎
1. Git rebase: what can go wrong? ↩︎
## Recent posts
