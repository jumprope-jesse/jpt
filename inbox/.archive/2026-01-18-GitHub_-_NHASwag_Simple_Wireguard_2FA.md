...
tags: ['Running']
created: 2024-05-11T18:29:00.000Z
---

# GitHub - NHAS/wag: Simple Wireguard 2FA

## AI Summary (from Notion)
- Project Overview: The document details the wag project, a tool that adds multi-factor authentication (MFA), route restrictions, and device enrollment to WireGuard.

- Key Features:
- Define routes requiring MFA authorization.
- Easy API for registering new clients.
- High availability and multiple MFA options (including WebAuthn and OIDC).

- Requirements:
- Dependencies include iptables and libpam.
- Must be run as root and requires a Linux kernel version newer than 5.9.

- Setup Instructions:
- Two installation options: binary release and building from source.
- Requires specific versions of libraries and tools.

- Management Commands:
- Commands for managing the wag server include start, cleanup, reload, version, and user/device management.

- User Guide:
- Instructions for installing wag, creating registration tokens, entering MFA, and signing into the management console.

- Configuration File:
- Detailed reference to various configuration options, including ACL rules, authentication methods, and clustering details.

- Limitations:
- Only supports clients with one AllowedIP, limited to IPv4 and Linux environments.

- Development and Testing:
- Includes instructions for creating custom templates, testing the application, and building releases.

- Donations and Support:
- Encourages donations to support the project and improve its sustainability.

- Interesting Fact:
- The document indicates that denial rules can now be defined in version updates, allowing for more granular access control.
...
1. Generate a wireguard private key with wg genkey set PrivateKey in the example config to it
1. Copy (or link) wag.service to /etc/systemd/system/ and start/enable the service
