...
tags: ['Running']
created: 2024-04-17T04:37:00.000Z
---

# Learn OAuth 2.0 by Building Your Own OAuth Client

## AI Summary (from Notion)
- Topic: Building an OAuth client using the Authorization Code grant type with Google.
- Main Steps in OAuth Process:
- User initiates login by choosing "Log in with Google."
- Website requests user permission from Google.
- User consents and receives an authorization code.
- Website exchanges the code for an access token from Google.
- Website retrieves user information with the access token.

- Key Components:
- CLIENT_ID & CLIENT_SECRET: Essential for secure communication between your server and Google.
- Redirect URI: Must be registered with Google to prevent unauthorized access.
- Scopes: Define what data the application is requesting from the user.

- Security Measures:
- Use of state parameter to prevent CSRF attacks.
- Keeping CLIENT_SECRET secure and using environment variables for management.

- Important Concepts:
- Resource Owner: The user whose data is accessed.
- User Agent: The browser or application interface used by the resource owner.
- OAuth Provider: Google in this case, which facilitates the OAuth flow.
- Authorization Server & Resource Server: Components responsible for issuing tokens and returning user information, respectively.

- Best Practices:
- Avoid building your own OAuth implementation from scratch due to potential security risks.
- Use well-tested open-source libraries for OAuth to ensure security and reliability.

- Next Steps: The guide hints at a follow-up exploration of OpenID Connect (OIDC), which builds upon OAuth and addresses additional authentication concerns.
...
1. Your Website Asks Google for Permission: Your website redirects the user to Google with a client ID that identifies your website to Google.
1. Google Seeks User Consent: Google displays a consent screen asking if the user agrees to share their Google details (like email and name) with your website.
1. User Consents and Receives a Code: Upon user agreement, Google redirects the user back to your website with an authorization code in the URL's query parameter.
1. Exchange Codes with Google: Your website's server sends this authorization code and a client secret to Google, verifying your website server's identity. Google then sends an access token back.
1. Your Website's Server Gets the User's Info: With this access token, your website's server can now ask Google for the user's information, like email and name. You then use this data to authenticate the user into your service.
## Setting Up Your OAuth Credentials

To start, you need Google to issue you a CLIENT_ID and CLIENT_SECRET. These tokens confirm that requests are originating from your server. Visit Google's authentication setup guide for detailed steps on obtaining these tokens.
...
To make the exchange, you make a POST request to Google's token endpoint; https://oauth2.googleapis.com/token. Below is an explanation about each part of the request.
