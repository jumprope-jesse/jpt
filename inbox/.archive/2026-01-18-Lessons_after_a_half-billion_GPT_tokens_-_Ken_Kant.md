...
tags: ['Running']
created: 2024-04-14T05:25:00.000Z
---

# Lessons after a half-billion GPT tokens - Ken Kantzer's Blog

## AI Summary (from Notion)
- Introduction: Ken Kantzer shares lessons learned from processing over 500 million tokens using OpenAI models, particularly GPT-4 and GPT-3.5, focusing on practical insights for using LLMs in a B2B context.

- Key Lessons:
- Less is More in Prompts: Overly detailed prompts can confuse GPT; simpler, more general prompts often yield better results.
- Avoid Premature Abstraction: Tools like Langchain may not be necessary; direct interaction with the chat API can suffice for most tasks.
- User Experience Innovations: Features like variable-speed typing enhance user interaction with LLMs.
- Challenges with Null Hypothesis: GPT struggles to return nothing when it doesn’t find results, often leading to hallucinations.
- Misunderstanding of Context Windows: GPT-4 has a larger input window compared to its output capacity, which limits its ability to return extensive lists.
- Vector Databases and RAG: These tools may not be beneficial for most users; traditional search methods could be more effective in business apps.
- Hallucination Reduction: When given full context, GPT reliably extracts information without fabricating details, especially in structured tasks.

- Future Outlook:
- Skepticism about achieving true general AI with current transformer models.
- GPT-4 is seen as genuinely useful despite marketing hype; it democratizes access to AI technology.
- Caution against overloading on information about LLM advancements; focus on significant updates like GPT-5.
- Expectations for GPT-5 suggest it may not revolutionize capabilities but could offer incremental improvements.

- Interesting Facts:
- 500 million tokens equate to approximately 750,000 pages of text.
- The use case for GPT is primarily in extracting structured data rather than generating new ideas or content.
