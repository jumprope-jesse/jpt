...
tags: ['Running']
created: 2024-05-18T02:25:00.000Z
---
...
- Purpose of Experts.js: A framework designed to simplify the creation and deployment of OpenAI's Assistants, enabling the development of Multi AI Agent Systems with enhanced memory and detail.

- Key Features:
- Supports the new Assistants API from OpenAI, advancing usability beyond the previous Chat Completions API.
- Allows Assistants to reference files and images, with the ability to manage extensive instructions (up to 256,000 characters).
- Integrates with up to 128 tools and utilizes a Vector Store API for efficient file searching.

- Object Structure:
- Assistants: Main AI agents created with a name, description, and instructions.
- Tools: Specialized Assistants that can perform specific tasks and interact with other Assistants.
- Threads: Managed context windows for storing messages and files, allowing for complex interactions.

- Installation: Simple installation via npm and straightforward usage with three primary objects to import.

- Development Process:
- Encourages asynchronous programming with event handling for real-time updates (e.g., streaming text or images).
- Provides a simple interface to ask questions or give instructions through the ask() function.

- Advanced Features:
- Implements lazy initialization of resources with the beforeInit() function.
- Allows for non-LLM Tools that can perform functions without relying on LLM capabilities.

- Examples Provided:
- Showcases various use cases, including product catalogs, streaming responses in Express, and handling messages with images.
- Demonstrates how to create and manage threads for context in conversations.

- Development Setup: Utilizes Dev Containers for easy setup, requiring API keys for OpenAI and image hosting services.

- Interesting Fact: The framework aims to alleviate the complexity of managing Run objects, facilitating a more user-friendly experience in building AI systems.

## Content (from Notion)

# Multi AI Agent Systems using OpenAI's Assistants API (Experts.js)
...
- Updates the assistants configurations to latest. (pending)
...
Tools are added to your Assistant via the addAssistantTool function. This function will add the tool to the assistant's tools array and update the assistant's configuration.
...
You can also create a thread with messages, files, or tool resources to start a conversation. We support OpenAI's thread create request body outlined in their Threads API reference.
...
By default, each Tool in Experts.js has its own thread & context. This avoids a potential thread locking issue which happens if a Tool were to share an Assistant's thread still waiting for tool outputs to be submitted. The following diagram illustrates how Experts.js manages threads on your behalf to avoid this problem:
