...
tags: ['Running']
created: 2024-02-07T05:03:00.000Z
---

# The Absolute Minimum Every Software Developer Must Know About Unicode in 2023 (Still No Excuses!) @ tonsky.me

## AI Summary (from Notion)
- Importance of Unicode: Unicode aims to unify all human languages for computer use, enabling global communication through standardized character encoding.
- UTF-8 Dominance: In 2023, UTF-8 is the prevailing encoding method, with a high probability of being used in most applications, making it essential for developers to understand how to use it correctly.
- Code Points and Characters: Unicode assigns unique numbers (code points) to characters, significantly expanding the character set beyond ASCII.
- Variable-Length Encoding: UTF-8 uses a variable-length encoding, allowing characters to be represented by one to four bytes, which enhances space efficiency for different languages.
- Grapheme Clusters: A grapheme is a unit of writing perceived by users as a single character, even if it consists of multiple code points. Understanding grapheme clusters is crucial for accurate string handling.
- Normalization: Different representations of the same character (e.g., composed vs. decomposed forms) require normalization before comparison to avoid mismatches.
- Locale Sensitivity: Unicode handling can vary by locale, affecting string operations like case conversion, highlighting the importance of context in text processing.
- Surrogate Pairs: UTF-16 uses surrogate pairs to encode characters outside of the Basic Multilingual Plane, complicating string handling.
- Error Handling: The Replacement Character (�) is used to signify Unicode errors when characters are not properly encoded or decoded.
- Yearly Updates: Unicode standards are updated annually, causing shifts in how characters and grapheme clusters are defined, necessitating regular updates in software handling text.
- Call to Action: Developers are encouraged to adopt libraries that handle Unicode properly, as they provide necessary functions for working with strings and grapheme clusters effectively.
- Overall Takeaway: Understanding Unicode and its implications is crucial for all software developers, as it affects text representation, processing, and user experience across applications.
...
1. C/C++/Java: use ICU. It’s a library from Unicode itself that encodes all the rules about text segmentation.
...
1. Roll your own. Unicode publishes rules and tables in a machine-readable format, and all the libraries above are based on them.
...
1. OR there’s also a pre-composed code point U+00C5 that does that for you.
...
October 2, 2023·Discuss on HackerNews
