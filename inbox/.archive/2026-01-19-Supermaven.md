...
tags: ['Running']
created: 2024-02-23T05:25:00.000Z
---

# Supermaven

## AI Summary (from Notion)
- Introduction of Supermaven: A new AI coding tool aimed at improving code completion for developers.
- Comparison with Competitors: Supermaven is positioned against established tools like Copilot, Codeium, and Tabnine, focusing on performance and cost-effectiveness.
- Key Features:
- 300,000-token Context Window: Allows Supermaven to consider extensive code context, improving accuracy over tools with smaller context windows.
- Speed: Custom infrastructure results in low latency; Supermaven shows significantly faster response times in tests.
- Understanding Code Edits: Unlike other tools that analyze files, Supermaven interprets the sequence of edits made, enhancing its ability to assist with refactoring.
- Market Position: Questions the dominance of large tech players in AI coding tools, suggesting that small startups can still compete by optimizing efficiency.
- User-Centric Focus: Aims to address common complaints about existing tools, such as struggles with unique or large codebases.
- Call to Action: Encourages developers to download and try Supermaven, highlighting excitement for user feedback.
...
At Supermaven we believe the question is not yet settled. The reason is the high serving costs of large models. While a model like GPT-4 offers unmatched suggestion quality, it's impossible to run on every keystroke (unless you charge users $1,000/month). Today, the deployment of an AI coding tool is limited just as much by the cost of the GPUs to run the tool as by the cost to develop the tool itself. That's why Copilot has made efforts to reduce costs by limiting the contexts in which they offer suggestions.
