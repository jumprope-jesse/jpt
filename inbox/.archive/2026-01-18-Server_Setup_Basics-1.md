...
tags: ['Running']
created: 2024-08-26T03:33:00.000Z
---

# Server Setup Basics

## AI Summary (from Notion)
- Server Setup Overview: Importance of establishing a solid foundation for self-hosting applications.
- SSH Security: Emphasis on using SSH keys instead of passwords for secure server access and disabling root login.
- User Management: Advocates for the principle of least privilege by creating separate users for each application, enhancing security and organization.
- Log Management: Importance of log rotation for system health, troubleshooting, and threat detection; suggests using tools like Grafana Loki for multi-server management.
- Backups: Highlights the necessity of testing backups and following the 3-2-1 rule (three copies, two storage types, one offsite).
- Network Security: Recommends using UFW and Fail2Ban to manage firewall rules and ban suspicious IPs.
- Web Server Recommendations: NGINX is preferred for serving static content and as a reverse proxy; provides sample configurations for different scenarios.
- SSL Certificates: Certbot is introduced as a straightforward tool for managing SSL certificates, including automatic renewal.
- Quality of Life Tools: Lists helpful tools such as Btop, Neoss, GoAccess, Midnight Commander, and NCDU for managing server resources and file systems.
- DNS Basics: Explains the role of DNS and suggests using Dynamic DNS for home hosting to handle changing IP addresses.
- Docker: Provides a balanced view of Docker's pros (consistency, isolation, ease of scaling) and cons (resource overhead, complexity in management).
- Final Takeaway: Importance of continuous learning and adapting server management strategies to meet specific needs.
